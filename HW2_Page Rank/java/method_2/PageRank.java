...
public class PageRank {
	...
	public static void main(String[] args) throws Exception {
	    ...
	    int iter = 0;
	    for (; iter < 20; ++iter){
	    	double deadEndContribution = getDeadEndContribution("/hw2/out/iter"+nf.format(iter), pagerank.NumberDeadEnd);
	    	double allContribution = getAllContribution("/hw2/out/iter"+nf.format(iter), iter, pagerank.NumberNode);
	    	pagerank.runPageRankCalculation("/hw2/out/iter"+nf.format(iter), "/hw2/out/iter"+nf.format(iter + 1),
	    			 						 ...
	    			 						 new Double(deadEndContribution), new Double(allContribution));
	    	 
	    }
	    // calculate page rank until converge
	    for (iter=19; ; ++iter){
	    	if (isConverge("/hw2/out/iter"+nf.format(iter) + "/" + PREFIX, "/hw2/out/iter"+nf.format(iter + 1) + "/" + PREFIX)) break;
	    	double deadEndContribution = getDeadEndContribution("/hw2/out/iter"+nf.format(iter), pagerank.NumberDeadEnd);
	    	double allContribution = getAllContribution("/hw2/out/iter"+nf.format(iter), iter, pagerank.NumberNode);
	...
	private static double getDeadEndContribution(String input, int numDeadEnd){
		double total = 1.0 * numDeadEnd;
		try{
            ...
            while (br.ready()) {
            	...
            	String[] token = line.split("\t");
            	if (token.length==2)
            		total += Double.parseDouble(token[1]);
            }
            br.close();
        }catch(Exception e){
        }
	...
	private static double getAllContribution(String input, int iteration, int numNode){
		double total = 0;
		if (iteration==0) return 1.0 * numNode;
		try{
            Path pathIn = new Path(input);
            FileSystem fs = FileSystem.get(new Configuration());
            BufferedReader br = new BufferedReader(new InputStreamReader(fs.open(pathIn)));
	...
	private void runPageRankCalculation(String inputPath, String outputPath, Integer numNode, Integer numDeadEnd, Double deadEndContri, Double allContri) throws IOException, ClassNotFoundException, InterruptedException {
		Configuration conf = new Configuration();
		conf.set("NumNode", numNode.toString());
		conf.set("NumDeadEnd", numDeadEnd.toString());
		conf.set("DeadEndContribution", deadEndContri.toString());
		conf.set("AllContribution", allContri.toString());
